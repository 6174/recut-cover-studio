...
成员清单
main.tsx: 工作台编排层；串行读取创作上下文和历史，上传参考素材，保存选择并将 Asset 交付要求发送给 Agent。
cover-composer.tsx: 左侧渠道、参考图、参考封面与补充要求输入表单。
cover-preview.tsx: 右侧本次真实生成的 Asset 预览；没有 Asset 时只显示空状态，绝不伪造封面。
asset-picker.tsx: 基于素材缓存的两类图片多选弹窗与引用条；支持上传本机图片后维护对应 assetId。
