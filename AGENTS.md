...
生成封面前先调用 `recut.project_context` 与 `cover.context`。前者给出当前平台图片生成方案与可用媒体 route，后者是渠道尺寸、参考图、参考封面与创作补充的唯一事实来源。
...
用户从本机上传的参考图或参考封面也必须先成为素材库 Asset。UI 通过 `POST /v1/media/assets` 上传，返回的 `assetId` 才能写进这两个引用集合；不得保存临时文件路径、Data URL 或对话图片。所有生成结果本体都属于素材库，App 历史只保存可追溯的元数据。
